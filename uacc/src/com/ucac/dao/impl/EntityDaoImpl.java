...
public class EntityDaoImpl implements EntityDao {
	...
	public <T> void BatchSave(List<T> lists, Class<T> t) throws ErrorException,
		...
		// 字段的所有数据保存在这里了
		// 字段的所有数据保存在这里了
		Map<String, Object> prop_11 = this.getAllFieldsValue(lists.get(0));

		String tableName = classElement.attributeValue("table");
		String sql = "insert into  " + tableName + "(";

		sql = this.createInsertSql(sql, prop_11, classElement);
		...
		try {
			...
			for (Iterator<T> iterator = lists.iterator(); iterator.hasNext();) {
				...
				Map<String, Object> props = this.getAllFieldsValue(tObject);
			 
				int pCount = 1;
			
				// 首先看看需不需要处理 primary key
				Element primaryKey = getColumnByPropertiesnName("id",
						classElement);
				Boolean idIncrement = Boolean.parseBoolean(primaryKey
						.attributeValue("auto_increment"));
				...
				while (fieldsIterator.hasNext()) {
					...
					if (props.get(name) != null) {
						...
						// 为非主外键
						if (prop != null && prop.attribute("key") == null) {
							this.setPreparedStatementByPropertieType(pstmt,
